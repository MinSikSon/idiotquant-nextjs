                    ...
                    <ul className='list-disc list-inside'>
                        {detail.map((item, index) => {
                            ...
                            if ('object' == typeof item) {
                                return (
                                    <ul key={`${subTitle}_${index}`} className='list-disc list-inside pl-4'>
                                        {item.map((subItem, subIndex) => {
                                            return (<li key={`${subTitle}_${index}_${subIndex}`}>{subItem}</li>);
                                        })}
                                    </ul>
                                );
                            }
                            return (<li key={`${subTitle}_${index}`}>{item}</li>);
                        })}
                    </ul>
                </div>
            </div>
...
export function ArticleTemplateSimple(props) {
    const title = props.title;
    const subTitle = props.subTitle;
    const link = props.link;
    ...
    return (
        <div className="px-4 sm:p-6 md:py-10 md:px-8">
            <div className="grid grid-cols-1 lg:max-w-5xl lg:gap-x-20 lg:grid-cols-2">
                ...
                <div className="grid gap-4 col-start-1 col-end-3 row-start-1 sm:mb-6 sm:grid-cols-4 lg:gap-6 lg:col-start-2 lg:row-end-6 lg:row-span-6 lg:mb-0">
                    <img src={props.img1} alt="" className="w-full h-20 object-cover rounded-lg sm:h-52 sm:col-span-2 lg:col-span-full" loading="lazy" />
                </div>
                <div className="mt-4 col-start-1 row-start-3 self-center sm:mt-0 sm:col-start-2 sm:row-start-2 sm:row-span-2 lg:mt-6 lg:col-start-1 lg:row-start-3 lg:row-end-4">
                    {/* <button type="button" className="bg-indigo-600 text-white text-sm leading-6 font-medium py-2 px-3 rounded-lg">Check availability</button> */}
                </div>
