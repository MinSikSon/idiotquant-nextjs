...
9. **화면은 로그 → 차트 → 운용 상황 → 버튼 순이다**(`bandsOf`). 위에서 아래로 "무슨 일이
   있었나 → 시장은 어떤가 → 나는 어떤 상태인가 → 무엇을 할까" 로 읽힌다. 두 칸 배치에서는
   ...
   - **띠는 겨루고, 순서가 정해져 있다.** 운용 상황과 버튼이 먼저 제 몫을 가져가고, 남는
     것을 차트와 로그가 나누되 **모자라면 로그가 양보한다** — 로그는 지나간 일이고 차트는
     지금 걸어야 할 판이다. 로그가 17% 를 못박던 시절에는 낮은 화면에서 차트가 130px 짜리
     띠로 눌렸다.
   - 버튼은 **두 줄이 들어가면 두 줄**(매매 셋 + NEXT), 아니면 넷을 한 줄로. 못박아 두면
     낮은 화면에서 NEXT 가 띠 밖으로 잘려 **턴을 넘길 수가 없다**(`ACTION_TWO_ROW`).
   - `buildFirm` 의 자리(6/18/52/70/100/116)는 띠가 190 은 될 때의 것이다. 좁거나 낮으면
     (`cramped`) 자산 숫자를 한 단계 줄이고 아래 셋을 위로 당긴다 — 안 그러면 손패가
     버튼 위로 넘쳐 나간다.
