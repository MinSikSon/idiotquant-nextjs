                        ...
                        props.resultList.map((element: CalculationResult, key: any) => {
                            return <ListItem className='p-1 border-2 border-gray-300 mb-1' key={key} onClick={(e) => props.handleOnClickResultList(e, key)}>
                                <div className='flex-col'>
                                    ...
                                    <div className="flex gap-1 pb-1">
                                        <Chip variant="outlined" value={`초기투자금: ${element['investmentAmount'].toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원 (${props.getInterestRateBenchmark(element['compounding'])})`} />
                                    </div>
                                    <div className="flex gap-1 pb-1">
                                        <Chip variant="outlined" value={`추가납입금: ${element['contributions'].toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원 (${props.getContributeRateBenchmark(element['frequency'])})`} />
                                    </div>
                                </div>
                            </ListItem>
                        })
