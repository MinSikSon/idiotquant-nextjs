...
function SearchContent() {
    ...
    const tickerFromUrl = searchParams.get("ticker");
    const isLoaded = tickerFromUrl === name && (data.kiChart.state === "fulfilled" || data.usSearchInfo.state === "fulfilled");
    ...
    if (!hasMounted) return <div className="w-full min-h-screen !bg-gray-50 dark:!bg-zinc-950" />;
    ...
    return (
        <div className="w-full min-h-screen !bg-gray-50 dark:!bg-zinc-950">
                        ...
                        <div className="flex flex-col border-t dark:border-zinc-800/50">
                            ...
                            <div className="flex items-center gap-3 px-4 py-2 border-b dark:border-zinc-800/30">
                                <div className="flex items-center gap-1.5 flex-shrink-0">
                                    ...
                                    <span className="text-[9px] font-black text-red-500 italic whitespace-nowrap uppercase">Hot 10</span>
                                </div>
                            ...
                            <div className="flex items-center gap-3 px-4 py-1.5">
                                <div className="flex items-center gap-1.5 flex-shrink-0">
                                    ...
                                    <span className="text-[9px] font-black text-zinc-400 italic whitespace-nowrap uppercase">Recent</span>
                                </div>
                                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-nowrap py-0.5">
            ...
            <main className="max-w-6xl mx-auto p-4 md:p-6">
                {error && <Callout intent={Intent.DANGER} className="mb-4">{error}</Callout>}

                {!tickerFromUrl ? (
                    <SearchGuide />
                ) : (
                    <>
                        {waitResponse && !isLoaded && <div className="py-10 text-center"><Spinner size={40} /></div>}
                            ...
                            <div className="flex justify-center mb-10">
                                <ModernTiltCard
                                    stock={krOrUs === "US" ? {
                                        ...
                                        per: data?.usDetail?.output?.perx ?? 0, pbr: data?.usDetail?.output?.pbrx ?? 0
                                    } : {
                                        code: tickerFromUrl, isUs: false, name, ticker: (corpCodeJson as any)?.[name]?.stock_code ?? "",
                                        ...
                                        per: data?.kiPrice?.output?.per ?? 0, pbr: data?.kiPrice?.output?.pbr ?? 0
                                    }}
