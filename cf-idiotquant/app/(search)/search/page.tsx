...
import { addKrMarketHistory, addUsMarketHistory, selectKrMarketHistory, selectUsMarketHistory } from "@/lib/features/searchHistory/searchHistorySlice";
...
export default function Search() {
  ...
  const [usStartDate, setUsStartDate] = useState<any>((new Date()).toISOString().split('T')[0]);
  // const [endDate, setEndDate] = useState<any>((new Date()).toISOString().split('T')[0]);
  ...
  function onSearchButton(stockName: any) {
    const result = us_tickers.find(ticker => ticker === stockName.toUpperCase());
    if (DEBUG) console.log(`[onSearchButton] result:`, result);
    const isKr = (undefined == result);
    if (true == isKr) {
      setKrOrUs("KR");
      ...
      dispatch(reqGetOverseasPriceQuotationsDailyPrice({ PDNO: stockName, FID_INPUT_DATE_1: formatDate(usStartDate) }));
...
`;
  ...
  function handleOnClickStarredIcon() {
    if (DEBUG) console.log(`[handleOnClickStarredIcon] kiInquireDailyItemChartPrice.output1.hts_kor_isnm:`, kiInquireDailyItemChartPrice.output1.hts_kor_isnm);
    if (DEBUG) console.log(`[handleOnClickStarredIcon] cfStarredStocks:`, cfStarredStocks);
    const prevStarredStocks: StarredStock[] = cfStarredStocks.starredStock;
    let newStarredStocks: StarredStock[] = [];
    const isStarred = (cfStarredStocks?.starredStock?.some(item => item.name === kiInquireDailyItemChartPrice.output1.hts_kor_isnm));
    if (DEBUG) console.log(`[handleOnClickStarredIcon] isStarred:`, isStarred);
    if (true == isStarred) {
      ...
      return <>
        <div className={`${fixed ? "z-50 w-full fixed top-0 left-0 bg-white dark:bg-black" : "relative"}`}>
          <div className="flex flex-col w-full">
            <SearchAutocomplete placeHolder={"🇰🇷:stock name 🇺🇸:stock ticker"} onSearchButton={onSearchButton} validCorpNameArray={all_tickers} />
...
`
              ...
              <div className="dark:bg-black dark:text-white text-xs p-3 shadow">
                <div className="dark:bg-black dark:text-white flex gap-2 font-mono">
                  ...
                  <div className="w-6/12 text-right">{(Number(kiInquireDailyItemChartPrice?.output1?.["stck_prpr"] ?? 0) * Number(kiInquireDailyItemChartPrice?.output1?.["lstn_stcn"] ?? 0)).toLocaleString()}</div>
                  <div className="w-2/12 text-left text-[0.6rem]">원</div>
                </div>
                <div className="dark:bg-black dark:text-white flex gap-2 font-mono">
...
`})()}
            ...
            {getUsNcav(finnhubFinancialsAsReported, kiUsMaretPriceDetail, [1.0, 1.5])}
            <div className="dark:bg-black dark:text-white text-xs p-3 shadow">
