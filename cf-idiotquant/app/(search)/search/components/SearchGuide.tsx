      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-16 lg:gap-10 w-full relative">
        ...
        {guides.map((item, i) => (
          <div key={i} className="flex flex-col items-center group">
            {/* Card Showcase with Hover Effects */}
            <div className="relative transform transition-all duration-500 group-hover:-translate-y-10 group-hover:scale-105 group-hover:rotate-2">
              ...
              {/* Grade Aura Effect */}
              <div className={cn(
                "absolute -z-10 inset-0 blur-[80px] rounded-full opacity-0 group-hover:opacity-60 transition-opacity duration-700",
                item.stock.grade.grade === 'SSS' ? 'bg-amber-400' : 'bg-blue-400'
              )} />
            </div>

            {/* Description Card */}
            <div className="mt-12 w-full max-w-[320px] p-6 rounded-2xl bg-white dark:bg-zinc-900 border-b-4 border-zinc-200 dark:border-zinc-800 shadow-lg group-hover:border-blue-500 transition-all duration-300">
              <div className={cn(
                "inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-3",
                item.stock.isUs ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30' : 'bg-blue-100 text-blue-600 dark:bg-blue-900/30'
              )}>
                {item.icon}
                {item.badge}
              </div>
              <h3 className="text-zinc-900 dark:text-zinc-100 font-black text-xl mb-2 italic flex items-center gap-2">
                {item.title}
              </h3>
      ...
      <footer className="mt-40 w-full flex flex-col items-center space-y-10">
        ...
        <div className="flex items-center justify-center gap-8 w-full">
          ...
          <div className="flex gap-4">
            {[1, 2, 3].map((n) => (
              <div key={n} className="w-2.5 h-2.5 rounded-full border-2 border-zinc-300 dark:border-zinc-700" />
            ))}
          </div>
        ...
        {/* License & Disclaimer */}
        <div className="text-center space-y-4 bg-zinc-100 dark:bg-zinc-900/50 px-10 py-8 rounded-[2rem] border border-dashed border-zinc-300 dark:border-zinc-800 transition-colors hover:border-blue-500/50">
          <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-[0.5em] flex items-center justify-center gap-2">
            <Zap className="w-3 h-3 fill-current" />
            Official QuantDex Analysis Platform
          </p>
          <div className="flex flex-wrap items-center justify-center gap-6">
            <Globe className="w-4 h-4 text-zinc-300 dark:text-zinc-600" />
            <p className="text-zinc-400 dark:text-zinc-500 text-xs font-bold italic tracking-tight">
              © 2026 IDIOTQUANT • MASTERING THE ALPHA
            </p>
            <Shield className="w-4 h-4 text-zinc-300 dark:text-zinc-600" />
          </div>
        </div>
      </footer>
    </div>
