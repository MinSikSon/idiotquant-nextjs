...
export const StockScoreGauge = ({ stock }: StockScoreProps) => {
    ...
    return (
        <div className="w-full">
            ...
            {/* 게이지 컨테이너 (h-3 기준 유지) */}
            <div className={`w-full h-3 rounded-full overflow-hidden ${isNegative ? "bg-red-100 dark:bg-red-900/30 ring-1 ring-red-500/50" : "bg-zinc-200 dark:bg-zinc-700"}`}>
                {/* 실제 게이지 바 */}
                <div
            ...
            <p className={`${isNegative ? "opacity-100" : "opacity-0"} text-[1rem] text-red-500 font-bold mt-1 text-right italic uppercase tracking-widest`}>
                Value at Risk
            </p>
        </div>
    );
