...
test("지정가 매수 — 갭 하락으로 더 싸게 시작했으면 시가에 산다 · 손절 — 갭 하락이면 시가에 팔린다(불리한 쪽). 이게 손절의 현실이다", () => {
    // ── 지정가 매수 — 갭 하락으로 더 싸게 시작했으면 시가에 산다
    {
    ...
    // ── 손절 — 갭 하락이면 시가에 팔린다(불리한 쪽). 이게 손절의 현실이다
    {
...
test("익절 — 갭 상승으로 더 비싸게 시작했으면 시가에 팔린다(유리한 쪽) · 값이 없거나 깨진 캔들에서는 아무것도 걸리지 않는다", () => {
    // ── 익절 — 갭 상승으로 더 비싸게 시작했으면 시가에 팔린다(유리한 쪽)
    {
        assert.equal(fillPrice(res("take_profit", 12000), candle(13000, 13500, 12800, 13200)), 13000);
    }

    // ── 값이 없거나 깨진 캔들에서는 아무것도 걸리지 않는다
    {
...
/* ── 걸 때 검사 ──────────────────────────────────────────────── */
test("종류·가격·수량이 성치 않으면 못 건다 · 가격·수량은 내림해서 정수로 들어간다", () => {
    // ── 종류·가격·수량이 성치 않으면 못 건다
    {
...
test("건 자리가 그대로 남는다 · 자리를 안 적으면 0번 — 종목이 하나뿐이던 시절 예약과 같다", () => {
    // ── 건 자리가 그대로 남는다
    {
    ...
    // ── 자리를 안 적으면 0번 — 종목이 하나뿐이던 시절 예약과 같다
    {
...
test("없는 자리에는 못 건다 · 예약은 ${MAX_RESERVATIONS}건까지", () => {
    // ── 없는 자리에는 못 건다
    {
