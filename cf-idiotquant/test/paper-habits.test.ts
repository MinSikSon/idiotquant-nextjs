...
test("진입 타이밍 — 섞이면 수량으로 가중된다 · 처분효과 — 이익은 빨리 손실은 오래 팔면 드러난다", () => {
    // ── 진입 타이밍 — 섞이면 수량으로 가중된다
    {
        // 앞 30일 상승, 뒤 30일 하락
...
test("처분효과 — 한쪽만 있으면 말하지 않는다 · 투입 강도 — 한 번에 넣은 비중과 관망 비율", () => {
    // ── 처분효과 — 한쪽만 있으면 말하지 않는다
    {
        const c = makeCandles(() => 10000);
...
test("한 주도 안 사고 관망만 한 판 · 회전율 — 시드 대비 총 매수대금", () => {
    // ── 한 주도 안 사고 관망만 한 판
    {
        const h = run([]);
