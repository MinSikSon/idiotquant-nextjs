...
test("새 판은 내려온 계단 위에서 — 다만 함정으로 떨어지면 아니다", () => {
    // ── 새 판은 걸어 들어갈 수 있는 칸에서 시작한다
    {
    ...
    // ── 새 판은 **올라가는 계단 위**에서 시작한다 — 지상에서 내려온 자리다
    {
...
test("1층은 증표로만 나가고, 아래층에서는 물러설 수 있다", () => {
    // ── 증표 없이는 **나갈** 수 없다 — 이기는 길은 1층의 그 한 칸이다
    {
        const s0 = newGame(5);
    ...
    // ── 아래층에서는 증표 없이도 물러설 수 있다 — 도망이 선택지에 있어야 한다
    {
...
test("되돌아간 층은 떠난 그대로다 — 지도도 물건도 안 불어난다", () => {
