...
test("왕실 근위대(Knight) 시작 장비 및 철벽의 자세 패시브 동작", () => {
    ...
    assert.equal(s.hero.origin, "knight");
    assert.equal(s.hero.hp, 14);
    assert.equal(s.hero.maxHp, 14);
    assert.equal(s.hero.str, 16);

    // 시작 장비 확인 (철퇴, 사슬 고리 갑옷, 식량)
    const mace = s.hero.pack.find((p) => p.kind === "weapon" && p.type === "mace");
    const armor = s.hero.pack.find((p) => p.kind === "armor" && p.type === "ring mail");
    ...
    // 기본 방어력
    const baseDef = heroDefense(s.hero);
    assert.equal(s.hero.guarded, false);

    // 제자리 대기(rest) 시 guarded 상태 활성화 및 방어력 +2
    const s1 = perform(s, { t: "rest" });
    assert.equal(s1.hero.guarded, true);
    assert.equal(heroDefense(s1.hero), baseDef + 2);

    // 이동 시 guarded 상태 해제
    const s2 = perform(s1, { t: "rest" }); // again
    assert.equal(s2.hero.guarded, true);
});

test("지하 도적(Rogue) 시작 장비 및 스탯 확인", () => {
    const s = newGame(2, {}, {}, {}, {}, "rogue");
    assert.equal(s.hero.origin, "rogue");
    assert.equal(s.hero.hp, 11);
    assert.equal(s.hero.str, 15);

    // 시작 장비 확인 (단검, 다트 10개, 순간이동 주문서, 식량)
    const dagger = s.hero.pack.find((p) => p.kind === "weapon" && p.type === "dagger");
    const dart = s.hero.pack.find((p) => p.kind === "weapon" && p.type === "dart");
    const teleport = s.hero.pack.find((p) => p.kind === "scroll" && p.type === "teleport");
...
test("방랑 연금술사(Alchemist) 시작 물약 100% 식별 및 회복 효과", () => {
    ...
    assert.equal(s.hero.origin, "alchemist");
    assert.equal(s.hero.hp, 12);
    assert.equal(s.hero.str, 14);
    ...
    // 회복 물약이 배낭에 있어야 함
    const healPot = s.hero.pack.find((p) => p.kind === "potion" && p.type === "healing");
    assert.ok(healPot, "체력 회복 물약이 있어야 함");
});

test("고서 연구자(Scholar) 시작 주문서/지팡이 식별 및 지팡이 8회 충전", () => {
    const s = newGame(4, {}, {}, {}, {}, "scholar");
    assert.equal(s.hero.origin, "scholar");
    assert.equal(s.hero.hp, 10);
    assert.equal(s.hero.str, 13);
    ...
    const wand = s.hero.pack.find((p) => p.kind === "wand" && p.type === "slow monster");
