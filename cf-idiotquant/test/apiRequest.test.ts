...
test("정상 JSON 은 본문 그대로, status 를 얹어서 · 워커가 HTML 오류 페이지를 주면 던지지 않고 읽히는 말로 바꾼다", async () => {
    // ── 정상 JSON 은 본문 그대로, status 를 얹어서
    {
    ...
    // ── 워커가 HTML 오류 페이지를 주면 던지지 않고 읽히는 말로 바꾼다
    {
...
test("네트워크가 끊기면 status 0 · 배열 응답은 배열 그대로 — 객체로 펼치면 모양이 깨진다", async () => {
    // ── 네트워크가 끊기면 status 0
    {
        offline();
...
test("status 를 실어 보낸다 — tradingStatus 가 404 로 '계정 없음'을 읽는다 · 빈 본문도 실패로 처리하고 터지지 않는다", async () => {
    // ── status 를 실어 보낸다 — tradingStatus 가 404 로 '계정 없음'을 읽는다
    {
