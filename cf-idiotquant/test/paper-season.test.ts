...
test("고객과 목표가 붙어 다니지 않는다 — 한 고객에 늘 같은 목표면 네 판이 전부다 · 캠페인이 없으면 고객도 목표도 없다 — 체험 운용은 예전 규칙 그대로", () => {
    // ── 고객과 목표가 붙어 다니지 않는다 — 한 고객에 늘 같은 목표면 네 판이 전부다
    {
    ...
    // ── 캠페인이 없으면 고객도 목표도 없다 — 체험 운용은 예전 규칙 그대로
    {
...
test("목록 안의 값만 나온다 · 벤치마크 초과 목표는 정해진 폭을 넘어야 한다", () => {
    // ── 목록 안의 값만 나온다
    {
        for (let i = 0; i < 30; i++) {
...
test("분산 목표는 이기고 + 세 자리 이상 · 참을성 목표를 관망으로 통과할 수는 없다", () => {
    // ── 분산 목표는 이기고 + 세 자리 이상
    {
