...
/* ── 세로 ───────────────────────────────────────────────────── */
test("세로 — 어떤 격자에서도 띠 높이가 음수가 아니다 · 세로 — 쌓인 띠의 합이 정확히 격자 세로다", () => {
    // ── 세로 — 어떤 격자에서도 띠 높이가 음수가 아니다
    {
...
test("세로 — 띠끼리 겹치지 않고 틈도 없다 · 세로 — 버튼과 고른 종목 판은 최소치를 지킨다", () => {
    // ── 세로 — 띠끼리 겹치지 않고 틈도 없다
    {
        for (const h of PORTRAIT_H) {
            const b = bandsOf(W, h);
...
test("흔한 폰에서는 버튼 띠에 제목 표시줄이 선다 · 제목을 접는 자리에서도 버튼은 누를 수 있는 크기다", () => {
    // ── 흔한 폰에서는 버튼 띠에 제목 표시줄이 선다
    {
...
test("어느 배치에서도 고른 종목 판이 창 안에 온전히 선다 · 세로 — 뉴스 창은 껍데기를 쓰고도 고객 한 줄이 남는다", () => {
    // ── 어느 배치에서도 고른 종목 판이 창 안에 온전히 선다
    {
...
test("세로 — 남는 세로는 목록이 받는다 · 세로 — 띠는 격자 폭을 다 쓴다", () => {
    // ── 세로 — 남는 세로는 목록이 받는다
    {
...
/* ── 가로 ───────────────────────────────────────────────────── */
test("가로 — 읽는 것은 왼쪽, 만지는 것은 오른쪽 · 가로 — 모든 띠가 격자 안에 있고 높이가 음수가 아니다", () => {
    // ── 가로 — 읽는 것은 왼쪽, 만지는 것은 오른쪽
    {
        for (const [w, h] of LANDSCAPE) {
            const b = bandsOf(w, h);
...
test("가로 — 두 칸이 저마다 세로를 정확히 채운다 · designSize 와 bandsOf 의 portrait 판단이 언제나 같다", () => {
    // ── 가로 — 두 칸이 저마다 세로를 정확히 채운다
    {
        for (const [w, h] of LANDSCAPE) {
            const b = bandsOf(w, h);
            if (b.portrait) continue;
    ...
    // ── designSize 와 bandsOf 의 portrait 판단이 언제나 같다
    {
...
test("designSize 가 낸 격자는 전부 그릴 수 있다 · 한글은 두 칸, 라틴은 한 칸으로 센다", () => {
    // ── designSize 가 낸 격자는 전부 그릴 수 있다
    {
...
test("한 낱말이 한 줄보다 길면 거기서 끊는다 · 칸에 들어가는 글은 접지 않는다", () => {
    // ── 한 낱말이 한 줄보다 길면 거기서 끊는다
    {
...
test("사이의 개수는 덩이 사이 + 머리 뒤 하나다 · 쌓은 높이는 덩이 + 사이 + 머리의 합이다", () => {
    // ── 사이의 개수는 덩이 사이 + 머리 뒤 하나다
    {
...
test("남는 세로를 나눠 가져도 칸을 절대 안 넘는다 · 사이는 기본 폭보다 좁아지지 않고, 상한을 넘지도 않는다", () => {
    // ── 남는 세로를 나눠 가져도 칸을 절대 안 넘는다
    {
    ...
    // ── 사이는 기본 폭보다 좁아지지 않고, 상한을 넘지도 않는다
    {
...
test("자리가 아무리 없어도 fitRows 는 끝난다 · 버리는 순서 — 없어도 되는 줄이 먼저, 덩이의 마지막 줄이 맨 나중", () => {
    // ── 자리가 아무리 없어도 fitRows 는 끝난다
    {
    ...
    // ── 버리는 순서 — 없어도 되는 줄이 먼저, 덩이의 마지막 줄이 맨 나중
    {
...
test("뒤쪽 덩이부터 버린다 — 앞이 결론이다 · 다 버린 덩이는 자리를 안 먹는다", () => {
    // ── 뒤쪽 덩이부터 버린다 — 앞이 결론이다
    {
