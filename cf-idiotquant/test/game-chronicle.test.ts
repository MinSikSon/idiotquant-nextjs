...
/* ── 적는다 ─────────────────────────────────────────────────── */
test("겪은 턴이 적히고, 안 겪은 턴은 null 이다 · 이미 적힌 것을 다시 적으면 **그대로 돌려준다** — 저장을 안 하려고", () => {
    // ── 겪은 턴이 적히고, 안 겪은 턴은 null 이다
    {
    ...
    // ── 이미 적힌 것을 다시 적으면 **그대로 돌려준다** — 저장을 안 하려고
    {
...
test("0턴 이하는 안 적는다 · 띠는 턴 순서대로 나오고 길이가 챕터 턴 수다", () => {
    // ── 0턴 이하는 안 적는다
    {
        assert.equal(notedCount(noteTurn(EMPTY_CHRONICLE, "y1998", 0, "bull")), 0);
    }

    // ── 띠는 턴 순서대로 나오고 길이가 챕터 턴 수다
    {
        let c: Chronicle = EMPTY_CHRONICLE;
...
/* ── ② 아는 것은 근거가 아니다 ──────────────────────────────── */
test("회귀는 다른 것은 그대로 지운다 — 남는 것이 연대기뿐이라는 확인 · 기억은 판정과 **다른 말**을 쓴다", () => {
    // ── 회귀는 다른 것은 그대로 지운다 — 남는 것이 연대기뿐이라는 확인
    {
...
test("떨어졌던·흐렸던 턴은 알아볼 값어치가 없고, 모르는 턴은 알아봐야 한다 · 연대기에 적히는 것은 엔진이 그 턴에 실제로 돌린 국면이다", () => {
    // ── 떨어졌던·흐렸던 턴은 알아볼 값어치가 없고, 모르는 턴은 알아봐야 한다
    {
    ...
    // ── 연대기에 적히는 것은 엔진이 그 턴에 실제로 돌린 국면이다
    {
...
test("뼈대는 회차를 넘어 같다 — 그래서 기억이 쓸모가 있다 · 모든 챕터의 국면 뼈대가 시드와 무관하다", () => {
    // ── 뼈대는 회차를 넘어 같다 — 그래서 기억이 쓸모가 있다
    {
