...
test("첫 회차와 그 뒤가 같은 말을 하지 않는다 · 시작 막이 지금 형편과 앞으로 올 것을 말한다", () => {
    // ── 첫 회차와 그 뒤가 같은 말을 하지 않는다
    {
...
test("없는 것은 줄을 안 만든다 — 0 은 정보가 아니다 · 회귀 막은 끝난 회차와 다음 회차를 둘 다 말한다", () => {
    // ── 없는 것은 줄을 안 만든다 — 0 은 정보가 아니다
    {
...
test("끝 막은 회귀하지 않는다고 말한다 · 집·회사 전환이 그 장의 연도를 말한다", () => {
    // ── 끝 막은 회귀하지 않는다고 말한다
    {
...
test("네 장이 저마다 다른 그림을 부르고, 그 키가 슬롯에 있다 · 결산이 수익률의 부호를 살린다", () => {
    // ── 네 장이 저마다 다른 그림을 부르고, 그 키가 슬롯에 있다
    {
...
test("결산이 에너지와 남은 빚을 그대로 싣는다 · 보수를 받았으면 결산이 그것을 말한다", () => {
    // ── 결산이 에너지와 남은 빚을 그대로 싣는다
    {
...
test("빚이 0 이면 숫자가 아니라 사건으로 말한다 · 새로 겪은 것이 없으면 그 줄을 아예 안 만든다", () => {
    // ── 빚이 0 이면 숫자가 아니라 사건으로 말한다
    {
...
test("한 번도 안 권한 판은 그 사실을 말한다 · 결산은 집으로 돌아오는 화면이다", () => {
    // ── 한 번도 안 권한 판은 그 사실을 말한다
    {
...
test("칸 크기가 그 자리에 정해진 크기와 같다 · 모든 칸이 시트 안에 있다", () => {
    // ── 칸 크기가 그 자리에 정해진 크기와 같다
    {
...
test("실제로 넣은 시트가 표가 말하는 크기와 같다 · 엔딩 넷은 모두 그릴 그림이 있다 — 판이 끝나는 자리라 자리표시로 두지 않는다", async () => {
    // ── 실제로 넣은 시트가 표가 말하는 크기와 같다
    {
    ...
    // ── 엔딩 넷은 모두 그릴 그림이 있다 — 판이 끝나는 자리라 자리표시로 두지 않는다
    {
