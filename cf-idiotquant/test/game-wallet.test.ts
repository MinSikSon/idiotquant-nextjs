...
/* ── 맡기는 돈 ──────────────────────────────────────────────── */
test("repayable 은 버튼이 적을 값이고, repay 가 실제로 내는 값과 같다 · 에너지가 높을수록 더 맡긴다 — 에너지가 곧 운용 규모다", () => {
    // ── repayable 은 버튼이 적을 값이고, repay 가 실제로 내는 값과 같다
    {
...
test("근거 없이 권하면 받아 주더라도 훨씬 적게 맡긴다 · 형편(purse)과 반응(gain/loss)은 다른 값이다 — 김 부장과 어머니가 그 증거다", () => {
    // ── 근거 없이 권하면 받아 주더라도 훨씬 적게 맡긴다
    {
    ...
    // ── 형편(purse)과 반응(gain/loss)은 다른 값이다 — 김 부장과 어머니가 그 증거다
    {
...
/* ── 알바 — 갇히지 않는 비율인가 ────────────────────────────── */
test("일당은 생활비 여러 턴치여야 한다 — 아니면 알바에 갇힌다 · 알바는 알아보기보다 비싸다 — 하루를 통째로 파는 일이다", () => {
    // ── 일당은 생활비 여러 턴치여야 한다 — 아니면 알바에 갇힌다
    {
    ...
    // ── 알바는 알아보기보다 비싸다 — 하루를 통째로 파는 일이다
    {
        assert.ok(PARTTIME_ENERGY > RESEARCH_COST);
    }
});

test("12턴을 알바로만 버텨도 에너지가 남지는 않는다 · 알바로 번 돈은 빚 앞에서 푼돈이다", () => {
    // ── 12턴을 알바로만 버텨도 에너지가 남지는 않는다
    {
...
/* ── 엔진에 물린 자리 ───────────────────────────────────────── */
test("1998 은 지갑 0 · 계좌 0 으로 열린다 · 맡은 돈은 챕터 시작 기준도 같이 올린다 — 받기만 해도 보수가 나오면 안 된다", () => {
    // ── 1998 은 지갑 0 · 계좌 0 으로 열린다
    {
    ...
    // ── 맡은 돈은 챕터 시작 기준도 같이 올린다 — 받기만 해도 보수가 나오면 안 된다
    {
