...
/* ── 비율 매수 (내 돈 기준) ───────────────────────────────────── */
test("살 수 없는 조건에서는 0 · 25% 를 네 번 누르면 네 자리에 고르게 담긴다", () => {
    // ── 살 수 없는 조건에서는 0
    {
...
test("현금이 모자라면 그만큼만 산다 · 최대(100%)는 내 돈이 아니라 현금 전액", () => {
    // ── 현금이 모자라면 그만큼만 산다
    {
...
test("현금이 모자라면 그만큼만 산다 · 1등분은 내 돈 전부 — 현금이 그만큼 있을 때", () => {
    // ── 현금이 모자라면 그만큼만 산다
    {
...
test("등분 수가 이상하면 0 · 1/4 과 25% 는 같은 몫이다 — 눈금만 다르다", () => {
    // ── 등분 수가 이상하면 0
    {
...
/* ── 비율 매도 ────────────────────────────────────────────────── */
test("전부(100%)는 남김없이 · 절반은 내림, 그래도 최소 한 주", () => {
    // ── 전부(100%)는 남김없이
    {
...
/* ── 비중 맞추기 ──────────────────────────────────────────────── */
test("가진 게 없으면 0 · 현금만 있을 때 60% 로 맞추면 60% 어치를 산다", () => {
    // ── 가진 게 없으면 0
    {
        assert.equal(sellPartQty(0, 50), 0);
    }

    // ── 현금만 있을 때 60% 로 맞추면 60% 어치를 산다
    {
...
test("너무 많이 담고 있으면 판다 · 0% 로 맞추면 전부 판다", () => {
    // ── 너무 많이 담고 있으면 판다
    {
...
test("맞춘 뒤 비중은 목표를 넘지 않는다 · 이미 목표에 있으면 아무것도 하지 않는다", () => {
    // ── 맞춘 뒤 비중은 목표를 넘지 않는다
    {
...
test("살 현금이 없으면 사자는 주문을 만들지 않는다 · 값이나 총자산이 0이면 셈이 성립하지 않는다", () => {
    // ── 살 현금이 없으면 사자는 주문을 만들지 않는다
    {
        assert.equal(rebalanceOrder({ targetPct: 100, price: 10_000, cash: 0, held: 500, totalAssets: 5_000_000 }), null);
    }

    // ── 값이나 총자산이 0이면 셈이 성립하지 않는다
    {
...
test("목표 비중은 0~100 밖으로 나가지 않는다 · 현금 50% 를 남기고 나머지를 넷으로 나눈다", () => {
    // ── 목표 비중은 0~100 밖으로 나가지 않는다
    {
        const price = 10_000, totalAssets = 10_000_000;
...
test("맞춘 뒤 자리마다 목표 금액에 가깝고, 넘지 않는다 · 쓰는 현금이 가진 현금을 넘지 않는다", () => {
    // ── 맞춘 뒤 자리마다 목표 금액에 가깝고, 넘지 않는다
    {
        const slots = four();
...
test("이미 균등하면 아무 주문도 없다 · 주식 0% 는 전부 판다", () => {
    // ── 이미 균등하면 아무 주문도 없다
    {
...
test("자리가 하나면 그 자리가 곧 주식 비중 · 값을 못 가져온 자리는 셈에서 빼고, 하나도 없으면 계획이 비어 있다", () => {
    // ── 자리가 하나면 그 자리가 곧 주식 비중
    {
    ...
    // ── 값을 못 가져온 자리는 셈에서 빼고, 하나도 없으면 계획이 비어 있다
    {
...
/* ── 빌려 팔 수 있는 만큼 ─────────────────────────────────────── */
test("비중은 0~100 밖으로 나가지 않는다 · 나온 주수는 담보 안에 들어가고, 한 주 더 빌리면 넘친다", () => {
    // ── 비중은 0~100 밖으로 나가지 않는다
    {
        const slots = four();
        assert.deepEqual(
    ...
    // ── 나온 주수는 담보 안에 들어가고, 한 주 더 빌리면 넘친다
    {
...
test("공매도 비율은 매수 비율과 같은 눈금이다 — 내 돈 기준 · 돈이 없으면 빌려 팔 수도 없다", () => {
    // ── 공매도 비율은 매수 비율과 같은 눈금이다 — 내 돈 기준
    {
