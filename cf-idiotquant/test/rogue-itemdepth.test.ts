...
test("장비만 낡는다 — 소모품은 깊은 층에서도 나온다", () => {
    // ── 깊은 층에 허름한 장비가 안 떨어진다 — 낡는 것은 장비뿐이다
    {
    ...
    // ── 26층에서도 체력 회복 물약과 식량이 나온다 — 소모품은 안 낡는다
    {
        const seen = new Set<string>();
...
test("1층은 1층짜리들뿐이고, 사다리는 단조롭다", () => {
    // ── 1층에서 뽑히는 무기·갑옷은 1층짜리들뿐이다
    {
...
test("이름은 전부 한글이고, 마이너스 손질은 없다", () => {
    // ── 모든 물건 이름이 한글이다 — 알아낸 것도, 모르는 것도
    {
    ...
    // ── 표에 없는 물건도 한글로 물러선다 — 옛 저장이 영어로 뜨지 않는다
    {
    ...
    // ── 예전 표에 있던 종류는 지금도 이름이 있다 — 저장은 판을 넘어 남는다
    {
