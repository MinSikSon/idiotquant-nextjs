...
//
// 1. 61종 아이템 (무기 13, 방어구 9, 주문서 10, 물약 8, 반지 8, 지팡이 11, 식량 1, 증표 1)
...
test("도감 대상은 정확히 61종이고 7개 카테고리로 나뉜다", () => {
    assert.equal(CODEX_ENTRIES.length, 61);

    const prog = itemCodexProgress();
    assert.equal(prog.totalCount, 61);
