...
//
// 1. 74종 아이템 (무기 19, 방어구 9, 주문서 10, 물약 9, 반지 14, 지팡이 11, 식량 1, 증표 1)
...
test("도감 대상은 정확히 74종이고 7개 카테고리로 나뉜다", () => {
    assert.equal(CODEX_ENTRIES.length, 74);

    const prog = itemCodexProgress();
    assert.equal(prog.totalCount, 74);
    assert.equal(prog.identifiedCount, 0);
    assert.equal(prog.masteredCount, 0);

    assert.equal(prog.byCategory.weapon.total, 19);
    ...
    assert.equal(Object.keys(WEAPONS).length, 19);
