...
test("같은 점수는 같은 등수를 나눠 갖고, 다음은 그만큼 건너뛴다", () => {
    // 700 · 700 · 700 · 100
...
test("똑같은 점수가 또 있으면 공동이라고 말한다 — 1등이라고 다 최고 기록이 아니다", () => {
...
test("분모는 이번 판을 포함한다 — 무덤은 화면이 뜨기 전에 이미 적혔다", () => {
...
test("옛 기록에는 증표 칸이 없다 — 「살아 돌아왔나」로 메운다", () => {
...
test("증표 칸이 `won` 을 이긴다 — 쥐고 죽은 판이 살아 돌아온 판처럼 세지지 않게", () => {
    const grave = tomb(0, 10, { won: false, amulet: true });
    assert.equal(tombScore(grave), 10000 + 500);
});

test("무덤을 적은 쪽이 목록을 돌려준다 — 이번 판이 딱 한 번 들어 있다", () => {
    withStorage(() => {
        const s = newGame(9);
        s.hero.gold = 380;
        s.deepest = 14;
...
test("서른 판까지만 남는다 — 분모가 무한정 늘지 않는다", () => {
    withStorage(() => {
        let last: Tomb[] = [];
