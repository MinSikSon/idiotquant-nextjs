...
test("창이 굴러가도 값이 누적되지 않는다 (긴 배열) · period 가 길이보다 크면 전부 null", () => {
    // ── 창이 굴러가도 값이 누적되지 않는다 (긴 배열)
    {
...
/* ── 돌파선 ──────────────────────────────────────────────────── */
test("돌파선은 오늘을 포함한다 — 오늘 신고가면 오늘 값이 상단 · 돌파선 창이 굴러간다", () => {
    // ── 돌파선은 오늘을 포함한다 — 오늘 신고가면 오늘 값이 상단
    {
...
/* ── 변동폭(ATR) ─────────────────────────────────────────────── */
test("True Range 는 전날 종가를 셈에 넣는다 — 갭은 고가−저가로 안 잡힌다 · 변동폭 밴드는 종가를 가운데 두고 위아래 대칭", () => {
    // ── True Range 는 전날 종가를 셈에 넣는다 — 갭은 고가−저가로 안 잡힌다
    {
...
test("mult 를 키우면 밴드가 넓어진다 · 앞쪽 구간은 세 도구 모두 null 로 비워 둔다", () => {
    // ── mult 를 키우면 밴드가 넓어진다
    {
