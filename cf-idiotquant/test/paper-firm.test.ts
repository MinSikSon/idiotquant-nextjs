...
test("유출입은 상·하한을 넘지 않는다 · 숫자가 아닌 값이 와도 0 으로 세고 터지지 않는다", () => {
    // ── 유출입은 상·하한을 넘지 않는다
    {
...
/* ── 맡은 돈 ─────────────────────────────────────────────────── */
test("성과가 먼저 곱해지고 그다음 고객이 들고 난다 · 크게 잃으면 굴릴 돈도 줄어든 채로 간다", () => {
    // ── 성과가 먼저 곱해지고 그다음 고객이 들고 난다
    {
...
/* ── 등급 ────────────────────────────────────────────────────── */
test("정산은 반기 전 금액을 기준으로 보수를 매긴다 · 맡은 돈이 늘면 등급이 올라간다", () => {
    // ── 정산은 반기 전 금액을 기준으로 보수를 매긴다
    {
...
/* ── 표기 ────────────────────────────────────────────────────── */
test("억·만 단위로 짧게 — 모바일 칸을 넘기지 않는다 · 최고점의 40% 아래로 떨어지면 문을 닫는다", () => {
    // ── 억·만 단위로 짧게 — 모바일 칸을 넘기지 않는다
    {
...
test("최고점을 모르면 판정하지 않는다 — 규칙이 없던 시절의 회사를 뒤늦게 벌하지 않는다 · 최고점은 이번 반기를 시작할 때의 맡은 돈도 후보다", () => {
    // ── 최고점을 모르면 판정하지 않는다 — 규칙이 없던 시절의 회사를 뒤늦게 벌하지 않는다
    {
...
test("최고점은 뒤로 가지 않는다 · 한 주도 안 산 반기는 성적과 무관하게 고객이 떠나고 성과보수가 없다", () => {
    // ── 최고점은 뒤로 가지 않는다
    {
    ...
    // ── 한 주도 안 산 반기는 성적과 무관하게 고객이 떠나고 성과보수가 없다
    {
...
/* ── 부서 ────────────────────────────────────────────────────── */
test("부서를 안 산 회사는 전부 기본값이다 · 리서치 도구만 산 회사는 규칙이 안 바뀐다", () => {
    // ── 부서를 안 산 회사는 전부 기본값이다
    {
        const none = perksOf([]);
...
test("예약 데스크는 걸 수 있는 예약을 늘린다 · 리스크 관리팀은 문 닫는 선을 낮추고 마진콜을 늦춘다", () => {
    // ── 예약 데스크는 걸 수 있는 예약을 늘린다
    {
        assert.ok(perksOf(["desk"]).maxReservations > BASE_RESERVATIONS);
    }

    // ── 리스크 관리팀은 문 닫는 선을 낮추고 마진콜을 늦춘다
    {
...
test("IR팀은 잘한 반기의 유입만 키운다 · 부서는 정산에 그대로 실린다", () => {
    // ── IR팀은 잘한 반기의 유입만 키운다
    {
        const ir = perksOf(["ir"]);
