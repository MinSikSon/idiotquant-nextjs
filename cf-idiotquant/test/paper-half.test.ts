...
/* ── 하루 넘기기 ─────────────────────────────────────────────── */
test("하루 넘기면 커서가 하나 는다 · 예약은 커서를 민 뒤 새로 열린 날에 본다", () => {
    // ── 하루 넘기면 커서가 하나 는다
    {
...
test("현금이 모자라 못 채운 예약은 지우지 않는다 · 예약은 건 자리의 캔들로만 판정한다", () => {
    // ── 현금이 모자라 못 채운 예약은 지우지 않는다
    {
        const dip = makeRound({
...
test("예약 지우기는 자리로 — 같은 조건을 두 번 걸 수도 있다 · 예약 데스크가 있으면 네 번째 예약도 걸린다", () => {
    // ── 예약 지우기는 자리로 — 같은 조건을 두 번 걸 수도 있다
    {
        let r = makeRound();
...
test("아무것도 안 사고 끝나면 수익률 0 · 값이 오른 만큼 수익률이 나온다 (수수료·세금만큼 덜)", () => {
    // ── 아무것도 안 사고 끝나면 수익률 0
    {
...
test("이월하면 넘길 만큼은 팔지 않고 종가로 다시 산 셈이 된다 · 이월 한도 — 자리 하나가 넘길 수 있는 몫은 시드를 자리 수로 나눈 만큼", () => {
    // ── 이월하면 넘길 만큼은 팔지 않고 종가로 다시 산 셈이 된다
    {
        let r = makeRound({ cursor: TOTAL });
        r = ok(halfTrade(r, { side: "buy", qty: 10, slot: 0 }));
    ...
    // ── 이월 한도 — 자리 하나가 넘길 수 있는 몫은 시드를 자리 수로 나눈 만큼
    {
...
/* ── 제출 ────────────────────────────────────────────────────── */
test("중도 포기하면 그날까지로 닫힌다 · 제출에는 캔들이 없다 — 서버가 이미 갖고 있다", () => {
    // ── 중도 포기하면 그날까지로 닫힌다
    {
        let r = makeRound();
        r = ok(halfTrade(r, { side: "buy", qty: 10, slot: 0 }));
...
/* ── 반기 목표 ───────────────────────────────────────────────── */
test("반기가 닫히면 목표 달성 여부가 판에 적힌다 · 캠페인 없이 굴린 판에는 목표가 없다", () => {
    // ── 반기가 닫히면 목표 달성 여부가 판에 적힌다
    {
...
/* ── 공매도 ─────────────────────────────────────────────────── */
test("목표 판정은 그 반기에 실제로 한 것으로 난다 · 공매도하면 담보가 묶이고 빌린 수량이 는다 — 날짜는 그대로", () => {
    // ── 목표 판정은 그 반기에 실제로 한 것으로 난다
    {
    ...
    // ── 공매도하면 담보가 묶이고 빌린 수량이 는다 — 날짜는 그대로
    {
        const r0 = makeRound();
...
test("현금보다 많이 빌려 팔 수는 없다 — 레버리지는 없다 · 값이 내려간 뒤 갚으면 번다", () => {
    // ── 현금보다 많이 빌려 팔 수는 없다 — 레버리지는 없다
    {
...
test("리스크 관리팀이 있으면 같은 손실을 버틴다 · 빌린 주식은 이월되지 않는다 — 마감 때 무조건 갚는다", () => {
    // ── 리스크 관리팀이 있으면 같은 손실을 버틴다
    {
...
test("공매도만 하고 끝낸 반기도 수익률이 잡힌다 · 공매도는 롱 비중(maxExposure)에 섞이지 않는다", () => {
    // ── 공매도만 하고 끝낸 반기도 수익률이 잡힌다
    {
        const drop = makeRound({
            cursor: TOTAL,
            holdings: [
    ...
    // ── 공매도는 롱 비중(maxExposure)에 섞이지 않는다
    {
        let r = makeRound({ cursor: TOTAL });
        r = ok(halfTrade(r, { side: "short", qty: 100, slot: 0 }));
        const fin = finishHalf(r);
