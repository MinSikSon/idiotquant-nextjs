 ...
 */
/**
 * `p` 에 `it` 을 얹어 한 뭉치로 만들 수 있나 — **겹쳐 쌓이는 무기**(화살·은화살·표창)끼리.
 * 줍기(`addToPack`)와 저장 되읽기(`mergeStacks`)가 이 하나를 본다. 손에 쥔 뭉치는 안 합친다
 * (쥔 것이 늘거나 사라지면 `weaponId` 가 엉뚱한 것을 가리킨다).
 */
function sameStack(hero: Hero, p: Item, it: Item): boolean {
    return (
        p.id !== it.id &&
        p.kind === "weapon" &&
        it.kind === "weapon" &&
        p.type === it.type &&
        !!WEAPONS[it.type]?.stack &&
 ...
 *
 * 연사가 붙는 것은 **쥔 발사기로 쏘는 탄약**뿐이다(석궁은 빼고 — `slowReload`). 레인저도
 * 활을 쥐고 화살을 쏠 때만 직업 연사 보너스를 받는다. 나머지는 늘 한 발이다 —
...
export function rapidFireOf(hero: Hero): { kind: "zap" | "throw"; item: Item } | undefined {
    ...
    return dart ? { kind: "throw", item: dart } : undefined;
}
