// 이력 — **회귀가 지우지 못하는 것.**
//
// `regress()` 는 돈도 에너지도 고객도 1997 로 되돌리고 `facts` 까지 비운다. 그래서 여태
...
export const END_REASONS: readonly EndReason[] = ["debtCleared", "debtRemains", "burnout", "ruined"];
...
export function normalizeCareer(raw: unknown): Career {
    ...
    return {
        ...
        endings: {
            ...
            // 옛 저장은 이 끝을 `trustLost` 로 적었다. 이름만 바뀌었을 뿐 같은 끝이라
            // 세어 둔 횟수를 버리지 않는다.
...
export function recordChapter(c: Career, sum: ChapterSummary): Career {
    return {
        ...
        bestEnergy: Math.max(c.bestEnergy, Math.round(sum.energy)),
