...
export default function StockListTable({
  ...
  return (
    <div className={cn("w-full space-y-6", className)}>
      ...
      <div className="relative overflow-x-auto rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-[#2a2a2a] dark:bg-[#0d0d0d]">
        <table className="w-full text-left text-[12px] border-collapse">
          <thead className="bg-neutral-50/80 text-neutral-500 dark:bg-[#1a1a1a]/50 dark:text-neutral-400">
            <tr>
              ...
              rows.map((row, idx) => (
                <tr key={`row-${idx}`} className="group hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    >
                      <div className="p-1.5 rounded-md bg-stone-100 dark:bg-[#2a2a2a] group-hover/btn:bg-blue-500 group-hover/btn:text-white transition-all">
                        <TrendingUp className="w-3.5 h-3.5" />
                      </div>
                      <span className="font-bold text-sm text-neutral-900 dark:text-neutral-100 group-hover/btn:text-blue-600 transition-colors tracking-tight">
                        {row.symbol}
                      </span>
                    </button>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={cn(
                      ...
                      Number(row.ncavRatio) > 1 
                        ...
                        : "bg-stone-100 text-neutral-500 dark:bg-[#2a2a2a] dark:text-neutral-500"
                    )}>
                      {row.ncavRatio}
                    </span>
                  </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-1.5">
                        {tokenAmounts.map(amt => (
                          <div key={`indiv-${amt}`} className="flex items-center rounded-md border border-neutral-200 dark:border-[#2a2a2a] bg-white dark:bg-[#1a1a1a] overflow-hidden shadow-xs">
                            <button
                              onClick={() => doTokenPlusOne(amt, row.symbol)}
                              className="px-2 py-1 hover:bg-stone-50 dark:hover:bg-[#2a2a2a] text-[10px] font-bold border-r border-neutral-100 dark:border-[#2a2a2a]"
                            >
                              {amt / 10000}만
                            </button>
                            <button
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-[#1a1a1a] animate-in zoom-in-95 duration-300">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-neutral-100 p-4 dark:border-[#2a2a2a]">
              <div className="flex items-center gap-2">
              ...
              <button 
                onClick={closeModal}
                className="rounded-full p-2 text-neutral-400 hover:bg-stone-100 dark:hover:bg-[#2a2a2a] transition-colors"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  <pre className="max-h-[300px] overflow-auto rounded-xl border border-neutral-200 bg-stone-50 p-4 font-mono text-[11px] leading-relaxed dark:border-[#2a2a2a] dark:bg-[#0d0d0d] dark:text-neutral-400">
                    {JSON.stringify(selected, null, 2)}
                  </pre>
                  <button 
                    onClick={() => navigator.clipboard.writeText(JSON.stringify(selected))}
                    className="absolute right-3 top-3 rounded-md bg-white p-2 shadow-sm border border-neutral-200 opacity-0 group-hover:opacity-100 hover:bg-blue-50 transition-all dark:bg-[#2a2a2a] dark:border-[#3a3a3a]"
                  >
            ...
            {/* Footer */}
            <div className="flex justify-end gap-3 border-t border-neutral-100 bg-neutral-50/50 p-4 dark:border-[#2a2a2a] dark:bg-[#1a1a1a]/50">
              <button
                onClick={closeModal}
...
function StatItem({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-stone-50 border border-neutral-100 dark:bg-[#1a1a1a] dark:border-[#2a2a2a]">
